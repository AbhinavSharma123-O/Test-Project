...
import type { UserProfile, Task, Session, MicroWin } from './types';
import {
  getUserProfile,
  saveUserProfile,
  getTasks,
...
export default function App() {
  ...
  useEffect(() => {
    if (profile?.preferences.dyslexiaFont) {
      document.body.style.fontFamily = 'OpenDyslexic, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
    } else {
  ...
  const handleResumeTask = (task: Task) => {
    ...
    const existingSession = sessions.find(s => s.taskId === task.id && !s.endedAt);

    let session: Session;
    if (existingSession) {
      session = existingSession;
    } else {
      const lastCompletedStep = task.steps.findLastIndex(s => s.isComplete);
      session = {
        id: crypto.randomUUID(),
        taskId: task.id,
        startedAt: new Date(),
  ...
  const handleStepComplete = () => {
    ...
    updatedTask.steps[currentSession.currentStepIndex].isComplete = true;
    updatedTask.steps[currentSession.currentStepIndex].completedAt = new Date();
  ...
  const handleStuck = () => {
    ...
    const updatedTask = { ...currentTask, stuckCount: currentTask.stuckCount + 1 };
    const updatedTasks = tasks.map(t => (t.id === updatedTask.id ? updatedTask : t));
    setTasks(updatedTasks);
    saveTasks(updatedTasks);
    setCurrentTask(updatedTask);
  ...
  const handlePause = () => {
    ...
    saveSessions([...updatedSessions.filter(s => s.id !== updatedSession.id), updatedSession]);
  ...
  if (state === 'welcome') {
    return <Welcome onGetStarted={() => setState('profile-setup')} />;
  }
